...
def call_coach_api(df_week,profile,num_accounts,df_all=None):
    ...
    if len(df_week)>0:
        ...
        else: week_pnl_val=round(df_week['R_Result'].sum()*500,2)
...
setTimeout(function(){{cm('pnl-0',{month_pnl},900);cm('pnl-1',{week_pnl},900);cm('pnl-2',{today_pnl},900);cp('pct-0',{month_pct},900);cp('pct-1',{week_pct},900);cp('pct-2',{today_pct},900);}},200);
        ...
        st.markdown(
            f'<div class="v3-panel" style="display:flex;justify-content:space-around;align-items:center;padding:24px;margin-top:16px;"margin-top:16px;>
...
# ============ CHARTS ============
    ...
    if rolling:
        ...
        rsvg=f'<svg viewBox="0 0 {rw} {rh}" style="width:100%;height:100px;display:block;"><defs><linearGradient id="rFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="rgba({RGB},0.15)"/><stop offset="100%" stop-color="rgba({RGB},0)"/></linearGradient></defs><line x1="0" y1="{by:.1f}" x2="{rw}" y2="{by:.1f}" stroke="{BORDER2}" stroke-width="1" stroke-dasharray="4,4"/>{"<path d="+chr(34)+rf+chr(34)+" fill="+chr(34)+"url(#rFill)"+chr(34)+"/>" if rf else ""}{"<path d="+chr(34)+rl+chr(34)+" fill="+chr(34)+"none"+chr(34)+" stroke="+chr(34)+ACCENT+chr(34)+" stroke-width="+chr(34)+"2"+chr(34)+" stroke-linecap="+chr(34)+"round"+chr(34)+" stroke-dasharray="+chr(34)+"2000"+chr(34)+" stroke-dashoffset="+chr(34)+"2000"+chr(34)+"><animate attributeName="+chr(34)+"stroke-dashoffset"+chr(34)+" from="+chr(34)+"2000"+chr(34)+" to="+chr(34)+"0"+chr(34)+" dur="+chr(34)+"2s"+chr(34)+" begin="+chr(34)+"0s"+chr(34)+" fill="+chr(34)+"freeze"+chr(34)+"/></path>" if rl else ""}</svg>'
...
# ============ COACH ============
    ...
    if cached:
        ...
        st.markdown(f'<div style="background:{BG2};border-radius:14px;padding:24px;text-align:center;margin-top:12px;"><div style="font-size:0.92em;color:{TEXT};font-weight:600;margin-bottom:6px;">Last week: {total_v} trades · {wr_v}% WR</div></div>',unsafe_allow_html=True)
    else:
