...
def get_all_trades():
    all_results = []
    has_more = True; start_cursor = None
    while has_more:
        payload = {}
...
def calc_stats(df_in):
    ...
    s['avg_win'] = round(r[r>0].mean(),2) if s['wins']>0 else 0; s['avg_loss'] = round(r[r<0].mean(),2) if s['losses']>0 else 0
...
You are NOT generic. You reference specific trades, specific dates, specific patterns, specific language from their notes. You have been watching Kaea trade and you know their tendencies. Be brutally honest with tough love but include genuine encouragement where earned. Kaea needs a coach who tells the truth.

Respond ONLY in this exact JSON format with no other text:
{{
  "debrief": "Your main weekly message to Kaea. 4-6 sentences. Specific, personal, brutally honest. Reference actual trades and numbers. End with one genuine encouraging line if earned.",
  "focus_points": [
  ...
  "behavioral_patterns": [
    "Specific pattern observed this week with evidence from trades",
...
# ============ CHARTS ============
    ...
    if rolling:
        ...
        rsvg=f"""<svg viewBox="0 0 {rw} {rh}" style="width:100%;height:100px;display:block;"><defs><linearGradient id="rFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="rgba({RGB},0.15)"/><stop offset="100%" stop-color="rgba({RGB},0)"/></linearGradient></defs><line x1="0" y1="{by:.1f}" x2="{rw}" y2="{by:.1f}" stroke="{BORDER2}" stroke-width="1" stroke-dasharray="4,4"/>{'<path d="'+rf+'" fill="url(#rFill)"/>' if rf else ''}{'<path d="'+rl+f'" fill="none" stroke="{ACCENT}" stroke-width="2" stroke-linecap="round" stroke-dasharray="2000" stroke-dashoffset="2000"><animate attributeName="stroke-dashoffset" from="2000" to="0" dur="2s" begin="0s" fill="freeze"/></path>' if rl else ''}</svg>"""
...
# ============ CALENDAR ============
    ...
    if st.session_state.selected_day:
        ...
        dtdf=df_main.dropna(subset=['Date','R_Result']).copy(); dtdf['day']=dtdf['Date'].dt.date; dtdf=dtdf[dtdf['day']==sd]
        ...
        for _,trade in dtdf.iterrows():
            rv=trade['R_Result']; lbl='Win' if rv>0 else ('Loss' if rv<0 else 'BE'); sg='+' if rv>0 else ''
            pair=trade.get('Pair','—'); pc2=GOLD_S if pair=='XAUUSD' else (PURPLE_S if pair=='NASDAQ' else ACCENT_SOFT)
...
# ============ COACH ============
    ...
    st.markdown(
        ...
        f'<div class="coach-avatar" style="width:56px;height:56px;border-radius:50%;background:rgba({RGB},0.08);border:1px solid rgba({RGB},0.15);display:flex;align-items:center;justify-content:center;flex-shrink:0;">'
        f'<span style="font-size:1.5em;">⚡</span></div>'
        f'<div>'
        f'<div style="font-size:1.05em;font-weight:700;color:{TEXT};">Your AI Trading Coach</div>'
        f'<div style="font-size:0.75em;color:{TEXT2};margin-top:3px;">Brutally honest · Tracks your patterns · Gets smarter every week</div>'
        f'</div>'
        f'</div>', unsafe_allow_html=True)
    ...
    if 'Risk Management' in df_lw.columns:
    ...
    week_pnl_v=lw_pnl(df_lw)
    week_r_v=round(df_lw['R_Result'].sum(),2) if len(df_lw)>0 else 0
    wins_v=int((df_lw['R_Result']>0).sum()) if len(df_lw)>0 else 0
    losses_v=int((df_lw['R_Result']<0).sum()) if len(df_lw)>0 else 0
    total_v=len(df_lw)
    ...
    # Best/worst trade
    if len(df_lw)>0:
        bt=df_lw.loc[df_lw['R_Result'].idxmax()]; wt=df_lw.loc[df_lw['R_Result'].idxmin()]
        tw1,tw2=st.columns(2)
        with tw1:
            bt_r=bt.get('R_Result',0); bt_pair=bt.get('Pair','?'); bt_session=bt.get('3SL Window','?')
    ...
    if cached:
        ...
        # Grade
        st.markdown(
            f'<div class="coach-card" style="
        background:{BG2};border-radius:16px;padding:20px 24px;margin-bottom:14px;display:flex;align-items:center;gap:16px;animation-delay:0.1s;">'
        ...
        # Behavioral patterns
        patterns=cached.get('behavioral_patterns',[])
        ...
        red_flags=[r for r in red_flags if r]
        if red_flags:
